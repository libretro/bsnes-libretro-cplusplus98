...
namespace nall {
  ...
  template<typename T> unsigned container_size(const T& object, typename mp_enable_if< cplusplus98::is_array<T> >::type = 0) {
    return sizeof(T) / sizeof(typename cplusplus98::remove_extent<T>::type);
  }
}
