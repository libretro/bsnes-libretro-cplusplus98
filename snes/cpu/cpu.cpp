...
static void cpu_default_write(unsigned addr, uint8 data) {
  cpu.wram[addr] = data;
}
...
void CPU::enable() {
  ...
  read = function<uint8 (unsigned)>(cpu_default_read);
  write = function<void (unsigned, uint8)>(cpu_default_write);
